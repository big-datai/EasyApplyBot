import time, random, csv, pyautogui, pdb, traceback, sys, os, yaml
...
logging.basicConfig(filename='use_cases.log', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
...
class LinkedinEasyApply:
    ...
    def handle_unknown_scenario(self, question):
        try:
            response = openai.Completion.create(
                engine="text-davinci-003",
                prompt=f"I am filling form need answer to this questions: {question}",
                max_tokens=50  # Adjust max_tokens as per your requirement
            )
            answer = response.choices[0].text.strip()
            self.log_issue(question, answer)
            return answer
        except Exception as e:
            logging.error(f"Failed to get response from ChatGPT: {e}")
            return "I'm sorry, I couldn't get instructions. Please try again later."

    def get_answer(self, question):
        if self.checkboxes[question]:
            return self.checkboxes[question]
        else:
            return self.handle_unknown_scenario(question)

    def handle_radio_select(self, question, options):
        answer = self.checkboxes.get(question)
        if answer:
            return answer
        else:
            return self.handle_unknown_scenario(question)

    def handle_select_dropdown(self, question, options):
        answer = self.checkboxes.get(question)
        if answer:
            return answer
        else:
    ...
    def log_issue(self, question, answer):
        log_entry = {
            "question": question,
            "answer": answer
        }
        with open('unprepared_questions.yaml', 'a') as log_file:
    ...
    def apply_jobs(self, location):
        ...
        for job_tile in job_list:
            ...
            try:
                ...
                logging.error(f'Error while parsing job title and link: {e}')  
                pass
            try:
                company = job_tile.find_element(By.CLASS_NAME, 'job-card-container__primary-description').text
            except Exception as e:
                logging.error(f'Error while parsing company name: {e}')  
                pass
            try:
                ...
                logging.error(f'Error while parsing poster name: {e}')  
                pass
            try:
                job_location = job_tile.find_element(By.CLASS_NAME, 'job-card-container__metadata-item').text
            except Exception as e:
                logging.error(f'Error while parsing job location: {e}')  
                pass
            try:
                apply_method = job_tile.find_element(By.CLASS_NAME, 'job-card-container__apply-method').text
            except Exception as e:
                logging.error(f'Error while parsing apply method: {e}')  
                pass
                ...
                try:
                    ...
                    while retries < max_retries:
                        try:
                            ...
                            logging.error(f'StaleElementReferenceException while parsing job card: {e}')             
                            retries += 1
                            continue
        ...
        if len(frm_el) > 0:
            for el in frm_el:
                ...
                try:
                    ...
                    elif 'served in the military' in radio_text:
                        answer = 'no'
                    elif 'have you ever worked for coalition before' in radio_text:
                        answer = 'no'
                    elif 'how did you hear about us' in radio_text:
                        answer = 'linkedin'
                    elif 'served or are you currently' in radio_text:
                        answer = 'no'
                    elif 'have you ever worked for' in radio_text:
                        answer = 'no'
                    elif 'what is your age range' in radio_text:
                        answer = '35-44'
                    elif 'what is your python expertise level' in radio_text:
                        answer = 'proficient'
                    elif 'are you a veteran' in radio_text:
                        answer = 'no'
                    elif 'what is your sexual orientation' in radio_text:
                        answer = 'heterosexual or straight'
                ...
                try:
                    ...
                    print(f'printing the question {question_text} the options in the form {options}')
                    if 'proficiency' in question_text:
                        ...
                        choice = ""
                        for option in options:
                            if 'i do not want to answer' in option.lower():
                                choice = option
                        self.select_dropdown(dropdown_field, choice)    
                    elif 'veteran' in question_text:
                        answer = 'I do not want to answer'
                        ...
                        for option in options:
                            if answer in option.lower():
                                choice = option
                        self.select_dropdown(dropdown_field, choice) 
                    elif 'gender pronouns' in question_text:
                        answer = 'He'
                        choice = ""
                        for option in options:
                            if answer in option.lower():
                        ...
                        choice = next((option for option in options if answer in option.lower()), options[-1])
                        self.select_dropdown(dropdown_field, choice)
                    else:
                        choice = next((option for option in options if 'yes' in option.lower()), options[-1])
                        self.select_dropdown(dropdown_field, choice)
                        self.record_unprepared_question("dropdown", question_text)
                    continue
                except:
            ...
            for el in frm_el:
                ...
                if 'email address' in text:
                    ...
                    try:
                        ...
                        logging.error(f"Country code {self.personal_info['Phone Country Code']} not found. {e}")
                    try:
                        ...
                        logging.error(f"Could not enter phone number: {e}")
                ...
                for pb in pb4:
                    try:
                        ...
                        try:
                            ...
                            logging.error(f"Error while parsing additional questions: {e}")
                        try:
                            self.send_resume()
                        except Exception as e:
                            logging.error(f"Error while sending resume: {e}")
                        if 'home address' in label:
                            self.home_address(pb)
                        elif 'contact info' in label:
                            self.contact_info()
                    except Exception as e:
                        logging.error(f"Error while filling up form: {e}")
        except Exception as e:
            logging.error(f"Error while filling up form: {e}")
            time.sleep(30)
    ...
    def record_unprepared_question(self, answer_type, question_text):
        ...
        try:
            ...
            logging.error(f"Special characters in questions are not allowed. Failed to update unprepared questions log: {question_text}")
